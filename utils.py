...
def create_pku_dataloader_from_dataset(tokenizer, dataset, fraction=1.0, batch_size=64):
    ...
    return dataloader
def create_privacy_violation_dataloader(tokenizer, dataset, fraction=1.0, batch_size=64):
    """
    Given the Privacy Violation dataset, create the dataloader on the harmful Q&A pairs.

    Args:
        tokenizer: Tokenizer.
        dataset: Loaded Privacy Violation dataset.
        fraction: Fraction of data to use (subsampling).
        batch_size: Batch size.

    Returns:
        Data loader of Privacy Violation harmful Q&A pairs.
    """

    # Preprocessing function for Privacy Violation dataset
    def preprocess(examples):
        ...
        for i in range(len(examples["prompt"])):
            ...
            # Calculate start index for the answer
            test_text = f"### Question: {prompt}\n ### Answer: "
            test_tokenized = tokenizer(
                test_text,
                truncation=True,
                ...
                return_tensors='pt'
                               ...
                               # Find the start location (exclude padding tokens)
            test_input_ids = test_tokenized["input_ids"][0]
            meaningful_tokens = (test_input_ids != 0).nonzero(as_tuple=True)[0]  # Get indices of non-padding tokens
            start_loc = len(meaningful_tokens)  # The index after the meaningful tokens in the prefix

            results["start_locs"].append(start_loc)
            #results["start_locs"].append(len(test_tokenized["input_ids"][0]) - 1)
    ...
    # Add labels and make it data loader.
    # Use DataCollatorForLanguageModeling for consistency with the original code.
    data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False)
...
def create_safe_dataloader(tokenizer, csv_file_path, batch_size=64):
    """
    Create a dataloader for the Safe dataset (safe.csv) with prompt-response pairs.

    Args:
        tokenizer: Tokenizer.
        csv_file_path: Path to the safe.csv file.
        batch_size: Batch size.

    Returns:
        Train, validation, and test dataloaders for the safe dataset.
    """
    # Load the dataset
    df = pd.read_csv(csv_file_path)
    prompts, responses = df["prompt"].values, df["response"].values
    ...
    test_dataloader = torch.utils.data.DataLoader(
        test_data, batch_size=batch_size, collate_fn=data_collator, shuffle=False
    )
...
def get_harmful_responses(dataset_split):
    ...
    Args:
        dataset_split: A split of the dataset (train or test) in prompt-response format.

    Returns:
        List of all harmful responses in the dataset split.
    """
    return dataset_split["response"]
...
def compute_reverse_kl(pretrained_model, current_model, batch, device):
    ...
    prob_q = prob_q.clamp(min=1e-12, max=1.0)
    prob_p = prob_p.clamp(min=1e-12, max=1.0)
...
#         assert 0 <= one_st < len(one_inp), "Start location is out of bounds!"
