...
from utils import (
    ...
    create_privacy_violation_dataloader,
...
def main(args) -> None:
    ...
    LORA = False
    access_token = "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"
    ...
    if "Llama-2-7b" in args.model_name:
        ...
        # model = get_peft_model(model, config)
        # model.print_trainable_parameters()
    elif "alpaca-7b" in args.model_name:
        LORA = True  # If you need LORA, set it to True
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            quantization_config=bnb_config,
            device_map="auto",
            torch_dtype=torch.float16,  # Use float16 consistently
        )
...
#     train_dataset = load_dataset("PKU-Alignment/PKU-SafeRLHF", split="train")
    train_dataset = load_dataset("csv", data_files={"train": "Privacy Violation_train.csv"})["train"]
    train_bad_loader = create_privacy_violation_dataloader(
        tokenizer, train_dataset, batch_size=args.batch_size
    )

    train_normal_loader, _, _ = create_safe_dataloader(
        tokenizer, csv_file_path="safe.csv", batch_size=args.batch_size
    )

    torch.save(train_normal_loader, "/home/hice1/aliu342/dataloader/train_normal_loader.pt")
    torch.save(train_bad_loader, "/home/hice1/aliu342/dataloader/train_bad_loader.pt")
...
#         for bad_batch, normal_batch in zip(train_bad_loader, train_normal_loader):
        for batch_idx, (bad_batch, normal_batch) in enumerate(zip(train_bad_loader, train_normal_loader)):
            if batch_idx >= 20:
                print(f"Stopping after 100 batches.")
                break
...
#     model.save_pretrained("save_model/peft_model_dir", from_pt=True)
#     logging.info("Peft model saved")
    if LORA:
