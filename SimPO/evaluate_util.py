...
def get_kl_divergence(model, oracle_model, eval_dataloader):
    ...
    kl_outputs = []
    oracle_model_device = oracle_model.device
    print(f"Oracle model device : {oracle_model_device}")
    print(f"Our model device : {model.device}")
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            outputs = model(**batch)
            outputs_oracle_model = oracle_model(input_ids.to(oracle_model_device),
                                                labels=labels.to(oracle_model_device),
                                                attention_mask=attention_mask.to(oracle_model_device))
                                                
            probs = F.log_softmax(outputs.logits, dim=-1)
            probs_oracle_model = F.log_softmax(outputs_oracle_model.logits, dim=-1)
            kl_divergence = nn.functional.kl_div(probs.to(oracle_model_device), probs_oracle_model.to(oracle_model_device), reduction='none', log_target=True)
